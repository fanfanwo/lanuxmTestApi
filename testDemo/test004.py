...
class test_Outgoingdata(unittest.TestCase):
    try:
        for i in range(1, test_par+1):
            par = '采集任务-自动构造%s' % i
            test_taskPort = 20000 + i
            test_type = random.choice(test_eventLevel)
            url = test_url + '/ics/v1.1/api/logtask/add_or_update?T=1655814956188'
            ...
            data = {
                  "taskName": par,
                  "taskType": 1,
                  "taskStatus": test_type,
                  "taskProtocol": "udp",
                  "taskPort": test_taskPort,
                  "deviceId": 1
                }
            ...
            if i == test_par:
                print(resp.text)
                if '200' in resp.text and '添加任务管理数据成功' in resp.text:
                    print('防护评估-添加任务管理数据成功！')
                else:
                    print('err_message:','防护评估-添加任务管理数据失败！')

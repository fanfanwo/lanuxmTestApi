...
class test_Aashboardshijian_wd_dict(unittest.TestCase):
    try:
        for i in range(1, test_par+1):
            par = '文档目录-自动构造%s' % i
            url = test_url + '/ics/api/knowledge/documentTree/createTree?T=1647501371789'
            ...
            if i == test_par:
                print(resp.text)
                if '200' in resp.text and '创建成功' in resp.text:
                    print('知识库-文档目录构造完成！')
                else:
                    print('err_message:','知识库-文档目录构造存在异常！')
